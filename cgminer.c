...
#define WORK_LEN  80
#define HASH_LEN  32
#define aes_data_str "6m2x0-fVr:%^h$~G"
#define aes_data_str_len 16
uint8_t aes_key[] = { 0x42, 0x6d, 0x60, 0x30, 0x31, 0x31, 0x32, 0x2a, 0x26, 0x16, 0x6a, 0x34, 0x2e, 0xd3, 0xa5, 0x13 };

static void aes_decrypt_cbc(uint8_t *key, uint8_t *iv, uint8_t *data, int len)
{
    struct AES_ctx ctx;
...
static void regen_hash(struct work *work)
{
    ...
    char aes_data[AES_KEY_SIZE] = { 0 };
    int salt_work_len = HASH_LEN + aes_data_str_len;
    char salt_work[WORK_LEN];

	flip80(swap32, data32);
	sha256(swap, 80, hash1);
	//sha256(hash1, 32, (unsigned char *)(work->hash));
	sha256(hash1, 32, dhash);

    memset(salt_work, 0, WORK_LEN);
    strncpy(aes_data, aes_data_str, aes_data_str_len);
    aes_decrypt_cbc(aes_key, iv, aes_data, aes_data_str_len);
    memcpy(&salt_work[0], aes_data, aes_data_str_len);
    memcpy(&salt_work[aes_data_str_len], hash1, HASH_LEN);
    sha256(dhash, WORK_LEN, (unsigned char *)(work->hash));
}
...
static void gen_hash(unsigned char *data, unsigned char *hash, int len)
{
	...
	sha256(data, len, dhash);
	sha256(dhash, 32, hash1);
